<aside class="bg-[#E5F3FF] sm:w-[24%] md:w-[20%] lg:w-[17%] h-screen flex flex-col">
  <?php $fill = false?>
    <a href="{{ route('home') }}">
        ...
        <!-- Portal Heading -->
        <h1
            class=" mt-2 mb-3 sm:text-[1.1rem] sm:ml-1.5 md:text-[1.4rem] lg:text-[1.3rem] 
          font-[Roboto-Serif]">
            Complain Portal</h1>
    </a>
    ...
    <script>
      function fillComplain(event){
        event.preventDefault();
        console.log("fill me")
        $fill = true;
      }
      function viewComplain(event){
        event.preventDefault();
        console.log("view me")
      }
      </script>
        ...
        <nav class="text-lg">
            <ul class="flex flex-col">
                <a href="{{ route('user.fillComp') }}" id='fill-complain' >
                    <li class="p-3 hover:bg-blue-500 hover:text-white {{$fill? "bg-black":""}}">
    ...
    <div class="mt-auto h-auto px-3 mb-2 text-2xl text-blue-500 hover:bg-blue-600 hover:text-white">
        <form method="POST" action="{{ route('logout') }}" class="">
            ...
            <a href="route('logout')"
                ...
                <svg class="inline ml-[-0.5] flex-grow hover:group-hover:translate-x-2 transform tansition-transform transition duration-200 ease-in-out"
                    xmlns="http://www.w3.org/2000/svg" height="1rem" viewBox="0 0 24 24">
                    <g transform="rotate(180 12 12)">
                        <g fill="none" stroke="Black" stroke-linecap="round" stroke-width="1.5">
                            <path
                                d="M9.002 7c.012-2.175.109-3.353.877-4.121C10.758
